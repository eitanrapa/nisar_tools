"""ASF Earthdata bulk downloader (the "direct by filename" path).

Adapted from the Alaska Satellite Facility's bulk-download script (generated by
ASF Vertex). It authenticates against Earthdata Login with a cookie jar and
streams files, transparently following the URS re-auth redirect and waiting on
the 202 burst-extraction service. :class:`bulk_downloader` here takes a list of
fully-formed URLs; URL construction (e.g. the NISAR GSLC path) lives in
:mod:`nisar_tools.download`.
"""
...
class bulk_downloader:
    def __init__(self, download_dir, files, parse_argv=False):
        # Define your custom download path here
        ...
        # Make sure we can write to our target directory
        if os.access(self.download_dir, os.W_OK) is False:
            print(f"WARNING: Cannot write to target path! Check permissions for {self.download_dir}")

        # For SSL
        self.context = {}

        # Check if user handed in a Metalink or CSV (CLI use only):
        if parse_argv and len(sys.argv) > 0:
            download_files = []
            input_files = []
    ...
    def get_cookie(self):
        ...
        print("Please enter Earthdata username & password:")
    ...
    def download_file_with_cookiejar(self, url, file_count, total, recursion=False):
        ...
        download_file = os.path.join(self.download_dir, os.path.basename(url).split('?')[0])
        ...
        try:
            ...
            if content_disposition and len(content_disposition):
                ...
                if possible_filename:
                    download_file = os.path.join(self.download_dir, possible_filename.pop())

            # Open our local file for writing and build status bar
            tf = tempfile.NamedTemporaryFile(mode='w+b', delete=False, dir=self.download_dir)
